@extends('layouts.app')
@section('title', 'Chi tiết đề tài phản biện')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h2 style="font-weight: bold">Chi tiết đề tài phản biện</h2>
                    </div>
                    <div class="card-body" style="font-size: 16px">
                        ...
                        @if ($deTai->sinhViens->count() == 1)
                            ...
                            @else
                                <p><strong>Sinh viên thực hiện:</strong> {{ $sinhVien->ho_ten }} ({{ $sinhVien->mssv }}) -
                                    Điểm:
                                    {!! $phanCongSV->diem_gvpb !== null ? number_format($phanCongSV->diem_gvpb, 1) : '<em>Chưa có</em>' !!}</p>
                            @endif
                        @else
                                ...
                                @foreach ($deTai->sinhViens as $sinhVien)
                                    ...
                                    @else
                                        <li>{{ $sinhVien->ho_ten }}
                                            ({{ $sinhVien->mssv }})
                                            - Điểm:
                                            {!! $phanCongSV->diem_gvpb !== null ? number_format($phanCongSV->diem_gvpb, 1) : '<em>Chưa có</em>' !!}
                                    @endif
                                @endforeach
                            </ul>
                        @endif
                        ...
                        <div class="text-center">
                            <a href="{{ route('cham_diem_phan_bien.danh_sach') }}" class="btn btn-secondary btn-lg">Quay
                                lại</a>
                        </div>
                    </div>
                </div>
